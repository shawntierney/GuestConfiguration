...
Import-Module $PSScriptRoot\DscOperations.psm1
...
function Test-GuestConfigurationMofResourceDependencies
{
    ...
    for ($i = 0; $i -lt $resourcesInMofDocument.Count; $i++) {
        if($resourcesInMofDocument[$i].CimInstanceProperties.Name -contains 'ModuleName' -and $resourcesInMofDocument[$i].ModuleName -ne 'GuestConfiguration') {
            ...
            Write-Warning -Message "The configuration '$configurationName' is using one or more resources outside of the GuestConfiguration module. Please make sure these resources work with PowerShell Core 6.0."
            break
        }
    }
}
...
function Copy-DscResources
{
    ...
    $resources | ForEach-Object {
        if($_.ImplementedAs -eq 'Binary') {
            ...
            Get-ChildItem $binaryResourcePath\* -Include *.sh | % { Convert-FileToUnixLineEndings -FilePath $_ }
...
function Copy-ChefInspecDependencies
{
    ...
    if($usingChefResource) {
        if($missingDependencies.Length) {
            ...
            $chefInspecProfiles | % { Copy-Item $_ $modulePath -Recurse -Force -ErrorAction SilentlyContinue }
        }
    }
    else {
